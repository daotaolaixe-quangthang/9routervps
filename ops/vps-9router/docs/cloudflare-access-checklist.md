...
## Scope
Protect only `ai.yourdomain.com`.

## DNS and SSL
1. Create `A/AAAA` record for `ai.yourdomain.com` to VPS public IP.
...
3. Domain: `ai.yourdomain.com` and path `/*`.
4. Policy:
...
1. Add WAF custom rule for `http.host eq "ai.yourdomain.com"`.
...
## Validation
1. Open private browser and visit `https://ai.yourdomain.com`.
