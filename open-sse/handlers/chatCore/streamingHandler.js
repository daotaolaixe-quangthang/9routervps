 ...
 */
export function buildOnStreamComplete({ provider, model, connectionId, apiKey, requestId, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest }) {
  const streamDetailId = `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, requestId, label: "STREAM USAGE" });
  };
